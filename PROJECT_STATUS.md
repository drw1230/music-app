...
2. **主题色彩功能已实现待发版**（2026-09-12，master 10639a1+）：真机冒烟通过（弹窗元素齐全、6 预设色显示、湖蓝预览 94 像素命中、✕ 回滚归零）。**待用户手动验收**：确认修改、HEX 输入、历史顶置、浅色模式预览。发版时 versionCode 9 / versionName 1.1.2
...
3. **需求清单**：① ~~播放页菜单"加入歌单"~~ ✅ 已完成；② ~~软件升级功能~~ ✅ 已完成（v1.1.0，二级菜单双源）；③ ~~听歌识曲~~ ✅ 已完成并**升级引擎**（v1.1.0：AHA/ACRCloud 免账号，识别后自动跳搜索）；④ ~~在线识曲（网页嵌入）~~ ❌ **已废弃并移除入口**（WebView 录音被回声消除削废，见本清单第 7 条）；⑤ **本地导出/导入备份文件**（收藏+歌单，已定案不含云端）⏳ 待开发；⑥ **调查"副本分身"现象** ⏳ 现象待用户确认；⑦ ~~主题模式修改~~ ✅ **已完成（主题色彩弹窗，待发版 v1.1.2）**：⋮菜单"深色模式"替换为"主题色彩"——浅色/深色二选一（不保留跟随系统）+ 官方浅紫固定首位色板 + 6 常用预设色 + HEX 自由输入（即时预览），选择期间临时换肤、确定落盘、✕/点外部回滚；主题色只替换 primary 一族（背景保持中性），历史记忆最多 9 个新色往前顶（DataStore `theme_color`/`color_history`）
